				...
				if( operation.wasSuccessful() ) {
					...
					if( workItemRecords.length > 0 ) {
						...
						_.each( workItemRecords, function( workItemRecord ) {
							...
							var workItemLink = Rally.nav.DetailLink.getLink( {
								record: workItemRecord,
								text: workItemRecord.data.FormattedID
							} );
							// Insert a blank target for EDP users so we don't navigate away from the app
							workItemLink = workItemLink.slice( 0, 3 ) + 'target="_blank"' + workItemLink.slice( 3 );
							this.addLabel( workItemBox, workItemLink );
							this.addLabel( workItemBox, '&nbsp;&nbsp;&nbsp;&nbsp;' );
